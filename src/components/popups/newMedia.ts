...
import EditMedia from './editNewMedia';
...
export default class PopupNewMedia extends PopupElement {
  ...
  private setTitle() {
    ...
    console.log('title set', key, title)
    replaceContent(title, i18n(key, args));
    // replaceContent(title, 'Send pablo');
  }
  ...
  private attachFiles() {
    ...
    Promise.all(promises).then(() => {
      ...
      this.iterate((sendFileDetails) => {
        console.log(sendFileDetails, 'files')
